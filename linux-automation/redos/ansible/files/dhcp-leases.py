...
for lease in report_dataset:
        print('│ ' + format(lease['ip_address'], '<15') + ' │ ' + \
                format(lease['hardware'].upper(), '<17') + ' │ ' + \
                format(str((lease['ends'] - now) if lease['ends'] != 'never' else 'never'), '>20') + ' │ ' + \
                format(lease['client-hostname'], '<17') + ' │')

print('├─────────────────┴───────────────────┴──────────────────────┴───────────────────┤')
print('│ Total Active Leases: ' + format(str(len(report_dataset)), '<57') + ' │' ) 
print('│ Report generated (UTC): ' + format(str(now), '<54') + ' │'  ) 
print('└────────────────────────────────────────────────────────────────────────────────┘')
