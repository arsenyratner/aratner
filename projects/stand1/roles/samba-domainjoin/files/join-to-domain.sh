...
if [ -n "$del_pc" ]
	...
	if [ -n "$v_kdc" ]
		then
		echo $v_pass_admin | adcli delete-computer -U $v_admin -S $v_kdc --domain=$v_domain $v_name_pc --stdin-password
		echo -e 'End Deleting PC'  &>> /var/log/join-to-domain.log
		echo -e "" &>> /var/log/join-to-domain.log
	else
		echo $v_pass_admin | adcli delete-computer -U $v_admin --domain=$v_domain $v_name_pc --stdin-password
		echo -e 'End Deleting PC'  &>> /var/log/join-to-domain.log
		echo -e "" &>> /var/log/join-to-domain.log
	fi
    exit
...
f_choce_pill() {
	...
	echo -e " Укажите (1 или 2): " choce_domain; exit 1
...
check_domain_name()
{
	...
	#$3 - v_name_pc
	#$4 - v_pass_admin
v_check=$(adcli show-computer -U "$1" --domain="$2" "$3" --stdin-password <<< "$4" 2>&1)
v_check=$(adcli show-computer -U "$1" --domain="$2" "$3" --stdin-password <<< "$4" 2>&1)
echo " Проверка аутентификации в домене:"  &>> /var/log/join-to-domain.log
echo "$v_check" | sed 's/:/: /g' &>> /var/log/join-to-domain.log
echo "$v_check" | sed 's/:/: /g' &>> /var/log/join-to-domain.log
...
freedom()
{
    ...
    if [ -n "$gui" ]
		...
		data_exit=( $(zenity --forms --separator=" " \
			...
			echo $v_pass_admin_gui | adcli delete-computer -vvv -U $v_leave_admin --domain=$v_domain  $v_delete_host --stdin-password  &>> /var/log/join-to-domain.log
			err_adcli=$?
      ...
      echo -e ' Введите имя контроллера домена или для продолжения нажмите ENTER: ' v_kdc; exit 1
			# read -p ' Введите имя администратора домена: ' v_leave_admin
      echo -e ' Введите имя администратора домена: ' v_leave_admin; exit 1
			# read -sp  " Введите пароль администратора домена: " v_pass_admin && echo
      echo -e " Введите пароль администратора домена: " v_pass_admin; exit 1
			# read -p ' Введите имя контроллера домена или для продолжения нажмите ENTER: ' v_kdc
      echo -e ' Введите имя контроллера домена или для продолжения нажмите ENTER: ' v_kdc; exit 1
			# read -p ' Введите имя администратора домена: ' v_leave_admin
      echo -e ' Введите имя администратора домена: ' v_leave_admin; exit 1
      ...
      echo -e " Введите пароль администратора домена: " v_pass_admin; exit 1
			...
			if [ -n "$v_kdc" ]
				...
				echo $v_pass_admin | adcli delete-computer -vvv -U $v_leave_admin --domain=$v_domain  $v_delete_host -S $v_kdc --stdin-password &>> /var/log/join-to-domain.log
				err_adcli=$?
				err_str_adcli=`cat /var/log/join-to-domain.log | tail -n 1`
			else
				# Удаление ПК из домена
				echo -e " " &>> /var/log/join-to-domain.log
				echo -e " ***Выполнение <adcli delete-computer>***" &>> /var/log/join-to-domain.log
				echo $v_pass_admin | adcli delete-computer -vvv -U $v_leave_admin --domain=$v_domain  $v_delete_host --stdin-password &>> /var/log/join-to-domain.log
				err_adcli=$?
				err_str_adcli=`cat /var/log/join-to-domain.log | tail -n 1`
			fi
			if [ $err_adcli -ne 0 ]; then
...
if [ -z "$result_realm" ]
   ...
   then (
      echo -e "Компьютер в домене! \nВывести компьютер из домена?"; exit 1
...
then
  ...
  echo -e ' Имя домена: ' v_domain; exit 1
  # read -p ' Имя домена: ' v_domain
  echo -e ' Имя домена: ' v_domain; exit 1
  ...
  while true; do
	  ...
	  echo -e ' Имя ПК: ' v_name_pc; exit 1
	  # read -p ' Имя ПК: ' v_name_pc
	  echo -e ' Имя ПК: ' v_name_pc; exit 1
  ...
  echo -e ' Имя администратора домена: ' v_admin_ipa; exit 1
  # read -p ' Имя администратора домена: ' v_admin_ipa
  echo -e ' Имя администратора домена: ' v_admin_ipa; exit 1
  f_realm_discover
  # read -sp  " Введите пароль администратора домена IPA: " v_pass_admin_ipa && echo
  echo -e " Введите пароль администратора домена IPA: " v_pass_admin_ipa; exit 1
  # read -sp  " Введите пароль администратора домена IPA: " v_pass_admin_ipa && echo
  echo -e " Введите пароль администратора домена IPA: " v_pass_admin_ipa; exit 1
  myAsk
  f_join_free_ipa
...
if [[ -z "$v_domain"  &&  -z "$v_name_pc"  &&  -z "$v_admin" && -z "$gui" &&  -z "$v_ou" ]]; then
	...
	if  [[ -z "$v_search_domain" ]]; then
		...
		echo
		echo -e ' Имя домена ['${GREEN}$v_search_domain${NC}']'; exit 1;
	...
	echo -e ' Имя администратора домена: ' v_admin; exit 1
	# read -p ' Имя подразделения ПК(OU=MyComputers) без кавычек или для продолжения нажмите ENTER:' v_ou
	echo -e ' Имя подразделения ПК(OU=MyComputers) без кавычек или для продолжения нажмите ENTER:' v_ou; exit 1
	# read -p ' Имя администратора домена: ' v_admin
	echo -e ' Имя администратора домена: ' v_admin; exit 1
	# read -p ' Имя подразделения ПК(OU=MyComputers) без кавычек или для продолжения нажмите ENTER:' v_ou
	echo -e ' Имя подразделения ПК(OU=MyComputers) без кавычек или для продолжения нажмите ENTER:' v_ou; exit 1
...
# Настройка для доверенного домена
#    idmap config TRUSTED : backend = rid
#    idmap config TRUSTED : range = 1000000-9999999
#    idmap config TRUSTED : base_rid = 6000
